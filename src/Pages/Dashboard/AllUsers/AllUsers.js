...
const AllUsers = () => {
    const { data: users = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://bikroy-bazar-server-maruf21hossain.vercel.app/users')
    ...
    const handleMakeAdmin  = id =>{
        fetch(`https://bikroy-bazar-server-maruf21hossain.vercel.app/users/admin/${id}`, {
            method :'PUT',
            headers: {
